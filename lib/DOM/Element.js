...
Element = (function () {
  ...
  (function () {
    ...
    Element.prototype.querySelectorAll = function (string) {
      ...
      select = function (queries, elements) {
        var query, selector, selected, element, childIndex, child;
        query = queries[0];
        selector = query[0];
        selected = [];
        element = elements;
        ...
        for (childIndex = 0; childIndex < element.childNodes.length; childIndex++) {
          ...
          if (!check(selector, child)) { continue; }
          selected.push(child);
        }
        // if (query.length > 1) {
        //   selected = select(query, selected);
        // }
        return selected;
      }
      string = String(string);
